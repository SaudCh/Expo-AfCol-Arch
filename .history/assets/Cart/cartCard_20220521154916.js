            ...
            <View style={{ flexDirection: "row", flex: 1, justifyContent: 'space-between' }}>
                <View style={{ marginLeft: 10 }}>
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        {subCategory.discount ? <Text style={{ fontSize: 15, textDecorationLine: "line-through" }}>Rs. {changeNS(price)}</Text> : <Text style={{ fontSize: 15 }}>Rs. {price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}.00</Text>}
