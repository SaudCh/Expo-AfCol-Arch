            ...
            <View style={{ flexDirection: "row", flex: 1, justifyContent: 'space-between' }}>
                <View style={{ marginLeft: 10 }}>
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        {subCategory.discount ? <Text style={{ fontSize: 15, textDecorationLine: "line-through" }}>Rs. {changeNS(price)}</Text> : <Text style={{ fontSize: 15 }}>Rs. {changeNS(price)}</Text>}
                        {subCategory.discount ? <Text style={{ fontSize: 15, color: COLORS.success }}> Rs. {changeNS(price - subCategory.discount)}</Text> : null}
                    </View>
                </View>

                <View style={{ alignItems: 'center' }}>
                    <Text style={{ fontSize: 15, fontWeight: 'bold', marginBottom: 5 }}>{subCategory.discount ? "Rs " + changeNS(actPrice * quantity) : changeNS(price * quantity)}</Text>
