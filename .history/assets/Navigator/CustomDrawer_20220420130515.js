...
import { StyleSheet, TouchableOpacity, ToastAndroid, ActivityIndicator } from "react-native";
...
export function CustomDrawer(props) {
    ...
    return (
        <View style={{ paddingTop: 39 }}>
            ...
            {category.map((res) => (
                <Text>
                    {res.subCategories.length === 0 ? "Hello" : "null"}
                </Text>
            ))}
            ...
            {isLoading ? <ActivityIndicator /> : (
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <Text>{item.subCategories}, {item.releaseYear}</Text>
                    )}
                />
            )}
