    ...
    return (
        <View style={{ ...styles.container }}>
            ...
            <View style={{ padding: 10 }}>
                ...
                {subCategory.discount ? <Text style={{ fontSize: 15, textDecorationLine: "line-through" }}>Rs. {price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}.00</Text> : <Text>Rs. {price}</Text>}
                {subCategory.discount ? <Text style={{ fontSize: 15, textDecorationLine: "line-through" }}>Rs. {(price - subCategory.discount).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}.00</Text> : null}
