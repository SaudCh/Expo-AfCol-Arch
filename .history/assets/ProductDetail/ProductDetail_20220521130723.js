        ...
        <View style={{ ...styles.container }}>
            ...
            <ScrollView style={{ padding: 10 }}>
                ...
                <View style={{ ...globalStyle.hStack, marginBottom: 5 }}>
                    {subCategory.discount ? <Text style={{ fontSize: 15, textDecorationLine: "line-through" }}>Rs. {(price * quantity).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}.00</Text> : <Text style={{ fontSize: 15 }}>Rs. {(price * quantity).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</Text>}
